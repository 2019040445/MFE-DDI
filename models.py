...
class Attention(nn.Module):
    ...
    def forward(self, z): # [997, 2, 997]
        w = self.project(z) # [997, 2, 1]
        beta = torch.softmax(w, dim=1) # [997, 2, 1]
        return (beta * z).sum(1) # ([997, 2, 997]).sum(1) --> [997, 997]
'''
    sum
'''
class MultiLevelDDI(nn.Module):
    ...
    def forward(self, d1_node, d1_attn_bias, d1_spatial_pos, d1_in_degree, d1_out_degree, d1_edge_input,
        ...
        '''
        x1 = torch.cat((output_1, d1_seq_fts_layer1),dim = 1)
        pad_size = 336 - 203  # 需要填充的大小  
        pad = (0, pad_size)  # 在末尾填充 pad_size 个零  
        x1_padded = F.pad(x1, pad, "constant", 0)  # 使用零填充
        
        x2 = torch.cat((output_2, d2_seq_fts_layer1),dim = 1)
        x2_padded = F.pad(x2, pad, "constant", 0)  # 使用零填充
        # print(x1_padded.shape,x2_padded.shape)

        fembeddings1 = torch.stack([x1_padded, d1_feature], dim=1)
        # print('fembeddings1: ',fembeddings1.shape)
        d1_emb_last = self.attention(fembeddings1)

        fembeddings2 = torch.stack([x2_padded, d2_feature], dim=1)
        # print('fembeddings2: ',fembeddings2.shape)
        d2_emb_last = self.attention(fembeddings2)
        # print(d2_emb_last.shape)
        final_fts_cat=torch.cat((d1_emb_last, d2_emb_last), dim=1)
        # print(final_fts_cat.shape)
        score = self.decoder(final_fts_cat)
...
#         final_fts_cat=torch.cat((d1_feature,d2_feature),dim=1)
#         # print(final_fts_cat.shape)
#         score = self.decoder_cat(final_fts_cat)
#         # assert False
...
#################################################################################################################
        ...
        # i = torch.cat((drug1_output, drug2_output), dim=1).permute(0, 2, 1) # -->[b, 256, 130]
        # i = self.input_dropout(i)
        # i = self.icnn(i) #[b, 16, 128]
        # f = i.view(drug1_n_graph, -1) #[b, 2048]
        # score = self.decoder(f)  #[b, 86]
